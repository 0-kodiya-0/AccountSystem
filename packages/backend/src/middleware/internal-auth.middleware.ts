 ...
 * - ca: [CA certificate]
 */
export const extractClientCertificateInfo = asyncHandler(
  (req: InternalRequest, res, next) => {
    try {
      ...
      if (cert && Object.keys(cert).length > 0) {
        ...
        logger.info(
          `Internal request authenticated with certificate: ${fingerprint}`,
        );
 ...
 */
export const validateInternalService = asyncHandler(
  (req: InternalRequest, res, next) => {
    ...
    if (!serviceId || !serviceSecret) {
      throw new AuthError(
        "Internal service credentials required",
        401,
