...
export const signup = asyncHandler(async (req: SignUpRequest, res, next) => {
  ...
  const generatedState = await generateOAuthState(
  ...
  next(new Redirect({ state: generatedState }, authUrls[provider]));
});
    ...
    passport.authenticate(
      ...
      async (err: Error | null, userData: ProviderResponse) => {
        try {
          ...
          const result = await AuthService.processSignInSignupCallback(
            userData,
            stateDetails,
          );
          ...
          if (result.authType === AuthType.SIGN_UP) {
            next(
              new Redirect({ state: result.state }, `../signup/${provider}`),
            );
          } else {
            next(
              new Redirect({ state: result.state }, `../signin/${provider}`),
            );
          }
        } catch (error) {
          logger.error(error);
  ...
  passport.authenticate(
    ...
    async (err: Error | null, result: ProviderResponse) => {
      try {
        ...
        const token = await verifyTokenOwnership(
          result.tokenDetails.accessToken,
          accountId,
        );
        ...
        const accessTokenInfo = await getTokenInfo(
          result.tokenDetails.accessToken,
        );
        ...
        logger.info(
          `Token updated for ${service} ${scopeLevel}. Processing success callback.`,
        );
...
export const requestPermission = asyncHandler(async (req, res, next) => {
  ...
  if (!account || !account.userDetails.email) {
    const callbackData: CallbackData = {
      code: CallbackCode.USER_NOT_FOUND,
      error: "Account not found or missing email",
  ...
  if (!validation.valid) {
    const callbackData: CallbackData = {
      code: CallbackCode.PERMISSION_ERROR,
      error: `Invalid scope name format: ${validation.errors.join(", ")}`,
      accountId: accountId as string,
    };
    return next(new Redirect(callbackData, getCallbackUrl()));
  }
  ...
  if (scopeNames.length === 0) {
    const callbackData: CallbackData = {
      code: CallbackCode.PERMISSION_ERROR,
      error: "At least one scope name is required",
      accountId: accountId as string,
    };
    return next(new Redirect(callbackData, getCallbackUrl()));
  }
  ...
  logger.info(
    `Initiating permission request for scope names: ${scopeNames.join(", ")}`,
  );
