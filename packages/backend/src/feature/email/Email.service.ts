...
export async function sendVerificationEmail(
  ...
  if (!email || !firstName || !token) {
    throw new ValidationError(
      "Email, firstName, and token are required for verification email",
    );
  }
...
export async function sendPasswordResetEmail(
  ...
  if (!email || !firstName || !token) {
    throw new ValidationError(
      "Email, firstName, and token are required for password reset email",
    );
  }
...
export async function sendPasswordChangedNotification(
  ...
  if (!email || !firstName) {
    throw new ValidationError(
      "Email and firstName are required for password changed notification",
    );
  }
...
export async function sendLoginNotification(
  ...
  if (!email || !firstName || !ipAddress || !device) {
    throw new ValidationError(
      "Email, firstName, ipAddress, and device are required for login notification",
    );
  }
...
export async function sendTwoFactorEnabledNotification(
  ...
  if (!email || !firstName) {
    throw new ValidationError(
      "Email and firstName are required for 2FA enabled notification",
    );
  }
