...
describe('Email Mock Service', () => {
  ...
  describe('getStatus', () => {
    it('should return email mock status and configuration', () => {
    ...
    it('should reflect current configuration', () => {
      ...
      expect(status.config.logEmails).toBe(true);
      expect(status.config.simulateDelay).toBe(true);
      expect(status.config.delayMs).toBe(500);
    });
  });
  ...
  describe('getEmailsByTestContext', () => {
    beforeEach(async () => {
      await emailMock.sendEmail(
        'user1@example.com',
        'Email 1',
    ...
    it('should return emails for specific test ID', () => {
      const result = EmailMockService.getEmailsByTestContext('test-001');

      expect(result.testId).toBe('test-001');
      expect(result.emails).toHaveLength(2);
      expect(result.count).toBe(2);
      expect(result.emails.every((email) => email.metadata?.testId === 'test-001')).toBe(true);
    });

    it('should apply limit to test context results', () => {
      const result = EmailMockService.getEmailsByTestContext('test-001', 1);

      expect(result.emails).toHaveLength(1);
      expect(result.count).toBe(1);
    });

    it('should return empty for non-existent test ID', () => {
      const result = EmailMockService.getEmailsByTestContext('non-existent');
  ...
  describe('getEmailsByFlow', () => {
    beforeEach(async () => {
      await emailMock.sendEmail(
        ...
        {
          metadata: { emailFlow: 'password-reset', flowStep: 'initial' },
        },
      );
      await emailMock.sendEmail(
        'user2@example.com',
        ...
        {
          metadata: { emailFlow: 'password-reset', flowStep: 'confirmation' },
        },
      );
      await emailMock.sendEmail(
        'user3@example.com',
        ...
        {
          metadata: { emailFlow: 'security', flowStep: 'alert' },
        },
      );
    });

    it('should return emails for specific flow', () => {
      const result = EmailMockService.getEmailsByFlow('password-reset');

      expect(result.flow).toBe('password-reset');
      expect(result.flowStep).toBe('all');
      expect(result.emails).toHaveLength(2);
      expect(result.count).toBe(2);
    });

    it('should filter by flow step', () => {
      const result = EmailMockService.getEmailsByFlow('password-reset', 'initial');

      expect(result.flow).toBe('password-reset');
      expect(result.flowStep).toBe('initial');
      expect(result.emails).toHaveLength(1);
      expect(result.emails[0].metadata?.flowStep).toBe('initial');
    });

    it('should apply limit to flow results', () => {
      const result = EmailMockService.getEmailsByFlow('password-reset', undefined, 1);

      expect(result.emails).toHaveLength(1);
      expect(result.count).toBe(1);
    });

    it('should return empty for non-existent flow', () => {
      const result = EmailMockService.getEmailsByFlow('non-existent');
  ...
  describe('getEnhancedStats', () => {
    beforeEach(async () => {
      await emailMock.sendEmail('user1@example.com', 'Email 1', EmailTemplate.PASSWORD_RESET, {
        FIRST_NAME: 'User1',
        RESET_URL: 'https://example.com/reset/token1',
      });
      await emailMock.sendEmail('user2@example.com', 'Email 2', EmailTemplate.LOGIN_NOTIFICATION, {
        FIRST_NAME: 'User2',
        LOGIN_TIME: '2024-01-01 10:00:00',
        IP_ADDRESS: '192.168.1.1',
        DEVICE: 'Chrome on Windows',
      ...
      emailMock.refreshConfig();
      try {
        await emailMock.sendEmail('fail@example.com', 'Fail', EmailTemplate.TWO_FACTOR_ENABLED, {
          FIRST_NAME: 'Fail',
          DATE: '2024-01-01',
        });
      } catch {
    ...
    it('should return enhanced statistics with summary', () => {
      ...
      expect(stats.summary.totalEmails).toBe(3); // 2 sent + 1 failed
      expect(stats.summary.successRate).toBe('66.67%'); // 2/3 * 100
      expect(stats.summary.mostUsedTemplate).toBeTruthy();
    });
    ...
    it('should calculate success rate correctly', () => {
      ...
      expect(stats.summary.successRate).toBe('66.67%');
    });
  });
