 ...
 */
export const authenticateGoogleApi = asyncHandler(
...
export const requireGoogleScope = (requiredScopeUrl: string) => {
  return asyncHandler(async (req: GoogleApiRequest, res, next) => {
    ...
    if (
      !requiredScopeUrl ||
      typeof requiredScopeUrl !== "string" ||
      requiredScopeUrl.trim().length === 0
    ) {
      throw new ServerError(
        "Invalid required scope configuration",
        500,
        ApiErrorCode.SERVER_ERROR,
      );
    }
...
export const requireGoogleScopes = (requiredScopeUrls: string[]) => {
  return asyncHandler(async (req: GoogleApiRequest, res, next) => {
    ...
    if (!Array.isArray(requiredScopeUrls) || requiredScopeUrls.length === 0) {
      throw new ServerError(
        "Invalid required scopes configuration",
        500,
        ApiErrorCode.SERVER_ERROR,
      );
    }
...
export const requireGoogleScopeName = (scopeName: string) => {
  return asyncHandler(
    async (req: GoogleApiRequest, res: Response, next: NextFunction) => {
...
export const googleApiAuth = (requiredScopeUrl: string) => {
  return [authenticateGoogleApi, requireGoogleScope(requiredScopeUrl)];
};
...
export const googleApiAuthByName = (scopeName: string) => {
  return [authenticateGoogleApi, requireGoogleScopeName(scopeName)];
};
...
export const googleApiAuthMultiple = (requiredScopeUrls: string[]) => {
  return [authenticateGoogleApi, requireGoogleScopes(requiredScopeUrls)];
};
...
export const googleApiAuthByNames = (scopeNames: string[]) => {
  return [authenticateGoogleApi, requireGoogleScopeNames(scopeNames)];
};
