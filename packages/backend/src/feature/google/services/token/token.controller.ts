 ...
 */
export const getTokenInfoController = asyncHandler(
  async (req: GoogleApiRequest, res, next) => {
 ...
 */
export const checkScopeAccess = asyncHandler(
  async (req: GoogleApiRequest, res, next) => {
    ...
    if (scopeNamesToCheck.length === 0) {
      throw new BadRequestError(
        'At least one scope is required. Use "scopes" (JSON array) or "scope" (single scope) query parameter',
      );
    }
    ...
    if (!validation.valid) {
      throw new BadRequestError(
        `Invalid scope name format: ${validation.errors.join(", ")}`,
      );
    }
    ...
    const results: Record<
      string,
      { hasAccess: boolean; scopeName: string; scopeUrl: string }
    > = {};
    ...
    const grantedCount = Object.values(results).filter(
      (result) => result.hasAccess,
    ).length;
