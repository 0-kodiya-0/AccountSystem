...
export interface GooglePermissionsDocument
  extends Document,
    Omit<GooglePermissions, "id"> {
...
// Utility methods
GooglePermissionsSchema.methods.addScopes = function (
  newScopes: string[],
): void {
...
GooglePermissionsSchema.methods.removeScopes = function (
  scopesToRemove: string[],
): void {
  const initialLength = this.scopes.length;
  this.scopes = this.scopes.filter(
    (scope: string) => !scopesToRemove.includes(scope),
  );
...
GooglePermissionsSchema.methods.hasAllScopes = function (
...
const initGooglePermissionsModel = async (connection: mongoose.Connection) => {
  return connection.model<GooglePermissionsDocument>(
    "GooglePermissions",
    GooglePermissionsSchema,
  );
};
