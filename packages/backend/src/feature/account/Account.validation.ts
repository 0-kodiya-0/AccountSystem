...
export function validateAccount(
  obj?: Omit<Partial<Account>, "id">,
): obj is Omit<Account, "id"> {
  if (
    obj !== null &&
    typeof obj === "object" &&
    ...
    if (obj.accountType === AccountType.Local && obj.provider) {
      throw new AccountValidationError(
        "Local accounts should not have a provider",
      );
    }
...
export function validateSignupRequest(request: SignupRequest): string | null {
  ...
  ValidationUtils.validateRequiredFields(request, [
    "firstName",
    "lastName",
    "email",
    "password",
