...
export class InternalNotificationHandler {
  ...
  private init(): void {
    ...
    this.internalNamespace.use((socket, next) => {
      try {
        ...
        const serviceId =
          socket.handshake.auth.serviceId || socket.handshake.query.serviceId;
        const serviceName =
          socket.handshake.auth.serviceName ||
          socket.handshake.query.serviceName;
        ...
        logger.info(
          `Internal service connected to notifications: ${serviceName} (${serviceId})`,
        );
        next();
      } catch {
  ...
  private handleInternalConnection(socket: InternalSocket): void {
    ...
    logger.info(
      `Internal notification socket connected: ${socket.id} for service ${serviceName}`,
    );
    ...
    socket.on("subscribe-account", (data: { accountId: string }) => {
      ...
      logger.info(
        `Service ${serviceName} subscribed to notifications for account ${data.accountId}`,
      );
    });
    ...
    socket.on("unsubscribe-account", (data: { accountId: string }) => {
      ...
      logger.info(
        `Service ${serviceName} unsubscribed from notifications for account ${data.accountId}`,
      );
    });
    ...
    socket.on("subscribe-accounts", (data: { accountIds: string[] }) => {
      ...
      logger.info(
        `Service ${serviceName} subscribed to ${data.accountIds.length} account notifications`,
      );
    });
    ...
    socket.on("disconnect", (reason) => {
      logger.info(
        `Internal notification socket disconnected: ${socket.id} (${serviceName}) - ${reason}`,
      );
    });
  }
