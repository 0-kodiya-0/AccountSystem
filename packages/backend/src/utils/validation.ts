...
export class ValidationUtils {
  ...
  static validateAccessToken(
    ...
    if (!accessToken) {
      throw new BadRequestError(
        `Access token is required for ${context}`,
        400,
    ...
    if (typeof accessToken !== "string") {
      throw new BadRequestError(
        `Access token must be a string for ${context}`,
        400,
        ApiErrorCode.TOKEN_INVALID,
      );
    }
    ...
    if (trimmedToken.length === 0) {
      throw new BadRequestError(
        `Access token cannot be empty for ${context}`,
        400,
        ApiErrorCode.TOKEN_INVALID,
      );
    }
    ...
    if (trimmedToken.length < 10) {
      throw new BadRequestError(
        `Invalid access token format for ${context}`,
        400,
        ApiErrorCode.TOKEN_INVALID,
      );
    }
  ...
  static validateRefreshToken(
    ...
    if (!refreshToken) {
      throw new BadRequestError(
        `Refresh token is required for ${context}`,
        400,
        ApiErrorCode.TOKEN_INVALID,
      );
    }
    ...
    if (typeof refreshToken !== "string") {
      throw new BadRequestError(
        `Refresh token must be a string for ${context}`,
        400,
        ApiErrorCode.TOKEN_INVALID,
      );
    }
    ...
    if (trimmedToken.length === 0) {
      throw new BadRequestError(
        `Refresh token cannot be empty for ${context}`,
        400,
        ApiErrorCode.TOKEN_INVALID,
      );
    }
    ...
    if (trimmedToken.length < 10) {
      throw new BadRequestError(
        `Invalid refresh token format for ${context}`,
        400,
        ApiErrorCode.TOKEN_INVALID,
      );
    }
  ...
  static validateScopeUrl(scope: string, context: string = "operation"): void {
    ...
    if (typeof scope !== "string") {
      throw new BadRequestError(
        `Scope must be a string for ${context}`,
        400,
        ApiErrorCode.VALIDATION_ERROR,
      );
    }
    ...
    if (trimmedScope.length === 0) {
      throw new BadRequestError(
        `Scope cannot be empty for ${context}`,
        400,
        ApiErrorCode.VALIDATION_ERROR,
      );
    }
    ...
    if (trimmedScope.length < 3) {
      throw new BadRequestError(
        `Invalid scope format for ${context}`,
        400,
        ApiErrorCode.VALIDATION_ERROR,
      );
    }
  ...
  static validateJwtToken(
    token: string,
    tokenType: string = "JWT token",
    context: string = "operation",
  ): void {
    if (!token) {
      throw new BadRequestError(
        `${tokenType} is required for ${context}`,
        400,
        ApiErrorCode.TOKEN_INVALID,
      );
    }
    ...
    if (typeof token !== "string") {
      throw new BadRequestError(
        `${tokenType} must be a string for ${context}`,
        400,
        ApiErrorCode.TOKEN_INVALID,
      );
    }
    ...
    if (trimmedToken.length === 0) {
      throw new BadRequestError(
        `${tokenType} cannot be empty for ${context}`,
        400,
        ApiErrorCode.TOKEN_INVALID,
      );
    }
    ...
    if (parts.length !== 3) {
      throw new BadRequestError(
        `Invalid ${tokenType} format for ${context}`,
        400,
        ApiErrorCode.TOKEN_INVALID,
      );
    }
    ...
    if (parts.some((part) => part.length === 0)) {
      throw new BadRequestError(
        `Invalid ${tokenType} structure for ${context}`,
        400,
        ApiErrorCode.TOKEN_INVALID,
      );
    }
   ...
   */
  static validateObjectIdWithContext(
    id: string,
    fieldName: string = "ID",
    context: string = "operation",
  ): void {
    if (!id) {
      throw new BadRequestError(
        `${fieldName} is required for ${context}`,
        400,
        ApiErrorCode.MISSING_DATA,
      );
    }
    ...
    if (!mongoose.Types.ObjectId.isValid(id)) {
      throw new BadRequestError(
        `Invalid ${fieldName} format for ${context}`,
        400,
        ApiErrorCode.VALIDATION_ERROR,
      );
    }
