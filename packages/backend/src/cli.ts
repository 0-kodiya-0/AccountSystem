    ...
    switch (arg) {
      ...
      case "--internal-port":
        if (nextArg && !nextArg.startsWith("-")) {
          ...
          if (isNaN(port) || port < 1 || port > 65535) {
            console.error(
              "Error: --internal-port must be a valid port number (1-65535)",
            );
            process.exit(1);
          }
...
function validateOptions(options: CLIOptions): void {
  ...
  if (options.internalSslKey && !fs.existsSync(options.internalSslKey)) {
    console.error(
      `Error: Internal SSL key file not found: ${options.internalSslKey}`,
    );
    process.exit(1);
  }
  ...
  if (options.internalSslCert && !fs.existsSync(options.internalSslCert)) {
    console.error(
      `Error: Internal SSL cert file not found: ${options.internalSslCert}`,
    );
    process.exit(1);
  }
  ...
  if (options.internalSslCa && !fs.existsSync(options.internalSslCa)) {
    console.error(
      `Error: Internal SSL CA file not found: ${options.internalSslCa}`,
    );
    process.exit(1);
  }
...
function applyEnvironmentOverrides(options: CLIOptions): void {
  ...
  if (options.internalSslCert) {
    process.env.INTERNAL_SERVER_CERT_PATH = path.resolve(
      options.internalSslCert,
    );
  }
...
async function main(): Promise<void> {
  ...
  try {
    ...
    console.error(
      `Failed to start backend server: ${error instanceof Error ? error.message : "Unknown error"}`,
    );
    process.exit(1);
  }
}
