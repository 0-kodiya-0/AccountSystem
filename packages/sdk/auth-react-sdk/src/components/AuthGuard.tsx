...
import { AuthGuardDecision, useAuthGuard, UseAuthGuardOptions } from '../hooks/useAuthGuard';

interface AuthGuardProps extends UseAuthGuardOptions {
...
export function AuthGuard({
  ...
  const { decision, redirectDestination, redirectReason } = useAuthGuard(authGuardOptions);
  ...
  if (decision === AuthGuardDecision.LOADING) {
    ...
    return (
      ...
      >
        ...
        />
        <p style={{ color: '#64748b', fontSize: '14px' }}>{redirectReason || 'Loading...'}</p>
        <style>{`
                    @keyframes spin {
  ...
  if (
    ...
    if (RedirectingComponent) {
      return <RedirectingComponent destination={redirectDestination} reason={redirectReason} decision={decision} />;
    }
    ...
    return (
      ...
      >
        ...
        />
        <p style={{ color: '#64748b', fontSize: '14px' }}>{redirectReason || 'Redirecting...'}</p>
        <style>{`
                    @keyframes spin {
                        0% { transform: rotate(0deg); }
                        100% { transform: rotate(360deg); }
  ...
  if (ErrorComponent) {
    return <ErrorComponent error={`Unknown auth state: ${decision}`} />;
  } else {
    return (
      <div
        style={{
