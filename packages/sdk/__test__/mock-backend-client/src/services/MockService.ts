...
export class MockService {
   ...
   */
  async getSystemHealth(includeDetails: boolean = true): Promise<SystemHealth> {
    try {
      return await this.httpClient.get<SystemHealth>('/health', {
  ...
  async getComponentHealth(component: string): Promise<ComponentHealth> {
    try {
      if (!component) {
        throw new AuthSDKError('Component name is required', ApiErrorCode.VALIDATION_ERROR);
      }
      return await this.httpClient.get<ComponentHealth>(`/health/${component}`);
    } catch (error) {
      throw this.handleError(error, `Failed to get health for component: ${component}`);
    }
  }
  ...
  async ping(): Promise<HealthPingResponse> {
    try {
      return await this.httpClient.get<HealthPingResponse>('/health/ping');
    } catch (error) {
      throw this.handleError(error, 'Failed to ping health endpoint');
    }
  }
  ...
  async getUptime(): Promise<UptimeResponse> {
    try {
      return await this.httpClient.get<UptimeResponse>('/health/uptime');
    } catch (error) {
      throw this.handleError(error, 'Failed to get uptime');
    }
  }
   ...
   */
  async getAvailableCheckers(): Promise<HealthCheckersResponse> {
    try {
      return await this.httpClient.get<HealthCheckersResponse>('/health/checkers');
    } catch (error) {
      throw this.handleError(error, 'Failed to get available checkers');
    }
  }
