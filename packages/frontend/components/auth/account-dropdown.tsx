...
export function AccountDropdown() {
  ...
  const otherAccounts = accounts.filter((acc) => acc.id !== currentAccountFromStore?.id).slice(0, 3);
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button variant="ghost" className="relative h-10 w-10 rounded-full">
          <UserAvatar name={displayName} imageUrl={currentAccount.userDetails.imageUrl} size="md" />
        </Button>
      </DropdownMenuTrigger>
      <DropdownMenuContent className="w-80" align="end" forceMount>
          ...
          <div className="flex flex-col space-y-2">
            <div className="flex items-center space-x-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-muted-foreground truncate">{currentAccount.userDetails.email}</p>
              </div>
              <div className="flex flex-col items-end space-y-1">
                <Badge variant={currentAccount.accountType === 'oauth' ? 'default' : 'secondary'} className="text-xs">
        ...
        {/* Account Actions */}
        <DropdownMenuItem onClick={() => router.push(`/accounts/${currentAccount.id}/settings`)}>
              ...
              <DropdownMenuItem key={account.id} onClick={() => handleSwitchAccount(account.id)}>
                <div className="flex items-center space-x-2 w-full">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground truncate">{account.userDetails.email}</p>
                  </div>
                </div>
              </DropdownMenuItem>
            ))}
