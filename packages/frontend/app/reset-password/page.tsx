...
import { usePasswordReset, PasswordResetStatus, AuthGuard } from '../../../sdk/auth-react-sdk/src';
...
export default function ResetPasswordPage() {
  ...
  const { status, message, error, isLoading, performReset } = usePasswordReset({
    onSuccess: (message) => {
  ...
  const onSubmit = async (data: ResetPasswordFormData) => {
    ...
    await performReset(token, data.password, data.confirmPassword);
  };
            ...
            <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-4">
              <div className="flex items-start space-x-3">
                ...
                <div className="space-y-1">
                  <p className="text-sm font-medium text-green-800 dark:text-green-200">Security Tip</p>
  ...
  if (!token || status === PasswordResetStatus.NO_TOKEN) {
    return (
      <AuthGuard
        allowGuests={true}
        requireAccount={false}
        ...
        errorComponent={ErrorDisplay}
      >
        <AuthLayout title="Invalid Reset Link" description="This password reset link is invalid">
          <div className="space-y-6">
            <div className="text-center space-y-4">
  ...
  if (status === PasswordResetStatus.INVALID_TOKEN || status === PasswordResetStatus.EXPIRED_TOKEN) {
    return (
      <AuthGuard
        allowGuests={true}
        requireAccount={false}
        ...
        errorComponent={ErrorDisplay}
      >
        <AuthLayout title="Reset Link Issue" description="There's an issue with your reset link">
          <div className="space-y-6">
            <div className="text-center space-y-4">
              <div className="mx-auto w-12 h-12 bg-destructive/10 rounded-full flex items-center justify-center">
                <XCircle className="w-6 h-6 text-destructive" />
              ...
              <div className="space-y-2">
                <h3 className="text-lg font-medium">
                  {status === PasswordResetStatus.EXPIRED_TOKEN ? 'Reset link expired' : 'Invalid reset link'}
                </h3>
                <p className="text-sm text-muted-foreground">
                  {error ||
                    (status === PasswordResetStatus.EXPIRED_TOKEN
  ...
  return (
    ...
    >
      <AuthLayout title="Reset Your Password" description="Enter your new password below">
        ...
        {/* Back to login */}
        <div className="text-center">
          <Button variant="ghost" className="text-sm" onClick={handleBackToLogin} disabled={isLoading}>
            Back to Sign In
          </Button>
        </div>
      </AuthLayout>
