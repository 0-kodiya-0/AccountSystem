...
export default function TwoFactorVerifyPage() {
  ...
  const { status, message, error, attemptsRemaining, isLoading, canRetry, verify, useBackupCode, clearError } =
    use2FAVerification({
      maxAttempts: 5,
  ...
  // Update lockout time - manage locally since hook doesn't provide it
  useEffect(() => {
    if (status === TwoFactorVerificationStatus.LOCKED_OUT && !lockoutTimeRemaining) {
      // Set initial lockout time if not provided by hook (5 minutes default)
      setLockoutTimeRemaining(300000); // 5 minutes in milliseconds
    }
  }, [status, lockoutTimeRemaining]);

  // Countdown timer for lockout
  useEffect(() => {
    if (lockoutTimeRemaining && lockoutTimeRemaining > 0) {
      const timer = setTimeout(() => {
        setLockoutTimeRemaining((prev) => (prev ? prev - 1000 : null));
      }, 1000);
  ...
  if (status === TwoFactorVerificationStatus.EXPIRED_SESSION) {
    return (
      <AuthLayout title="Session Expired" description="Your 2FA session has expired">
  ...
  if (status === TwoFactorVerificationStatus.SUCCESS) {
    return (
      <AuthLayout title="Authentication Successful" description="You have been successfully signed in">
        <div className="space-y-6">
          <div className="text-center space-y-4">
            ...
            <div className="space-y-2">
              ...
              <p className="text-sm text-muted-foreground">{message || 'You have been successfully authenticated.'}</p>
            </div>
          </div>

          <div className="text-center">
            <p className="text-xs text-muted-foreground">You will be redirected to your dashboard shortly...</p>
          </div>
        </div>
      </AuthLayout>
    );
  ...
  if (status === TwoFactorVerificationStatus.LOCKED_OUT) {
    ...
    return (
      <AuthLayout title="Too Many Failed Attempts" description="Your account has been temporarily locked">
        <div className="space-y-6">
          <div className="text-center space-y-4">
            ...
            <div className="space-y-2">
              ...
              <p className="text-sm text-muted-foreground">
                Too many failed verification attempts. Please try again in {minutesRemaining} minute
          ...
          <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-4">
            <div className="flex items-start space-x-3">
              ...
              <div className="space-y-1">
                <p className="text-sm font-medium text-red-800 dark:text-red-200">Security Lockout</p>
          ...
          <div className="space-y-3">
            ...
            <div className="text-center">
              <Button variant="ghost" size="sm" onClick={() => router.push('/forgot-password')}>
  ...
  return (
    <AuthLayout
      ...
      description={
        isBackupCode ? 'Enter a backup recovery code' : 'Enter the verification code from your authenticator app'
      }
    >
      <div className="space-y-6">
        ...
        <div className="text-center space-y-4">
          <div className="mx-auto w-12 h-12 bg-primary/10 rounded-full flex items-center justify-center">
            {isBackupCode ? <Key className="w-6 h-6 text-primary" /> : <Smartphone className="w-6 h-6 text-primary" />}
          </div>
          <div className="space-y-2">
            <h3 className="text-lg font-medium">{isBackupCode ? 'Enter backup code' : 'Enter authenticator code'}</h3>
            <p className="text-sm text-muted-foreground">
              {isBackupCode
        ...
        {!isBackupCode ? (
          <form onSubmit={verificationForm.handleSubmit(onVerificationSubmit)} className="space-y-4">
            ...
            <Button type="submit" className="w-full" loading={isLoading} disabled={isLoading || !canRetry}>
              Verify and Sign In
            </Button>
          </form>
        ) : (
          <form onSubmit={backupForm.handleSubmit(onBackupSubmit)} className="space-y-4">
            <div className="space-y-2">
              ...
              {backupForm.formState.errors.backupCode && (
                <p className="text-sm text-destructive">{backupForm.formState.errors.backupCode.message}</p>
              )}
              {error && <p className="text-sm text-destructive">{error}</p>}
            </div>
